...
export default function CoachApplyPage() {
  ...
  const fetchMe = async () => {
    try {
      ...
      if (data?.application?.status) setStatus(String(data.application.status))
      ...
      <div className="max-w-screen-md mx-auto pb-24 px-4 pt-6 space-y-4">
        <GlassCard className="p-4">
          ...
          {status ? (
            ...
            <div className="mt-3 text-sm text-slate-300/70">Not submitted yet.</div>
          )}
          {error ? <div className="mt-3 text-sm text-red-200">{error}</div> : null}
        </GlassCard>

        <GlassCard className="p-4">
          <SectionHeader title="Profile details" subtitle="What users will see" />
          <div className="mt-4 space-y-3">
            ...
            <SectionHeader title="Contact" subtitle="Used for booking notifications (email required)" className="pt-2" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
