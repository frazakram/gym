...
export function HeatMap({ data, weeks = 8, onCellClick }: HeatMapProps) {
    ...
    const cellStyle = (value: number): React.CSSProperties => {
        ...
        return { backgroundColor: 'var(--primary-light)', border: '1px solid #b79bff', boxShadow: '0 0 4px rgba(139,92,246,0.4)' }
    }
    ...
    return (
        <div className="w-full">
                ...
                <div className="flex flex-col gap-[5px] shrink-0 w-3.5">
                    {DAY_LABELS.map((label, i) => (
                        <div key={i} className="h-3.5 flex items-center justify-end">
                            <span className="text-xs text-white/70 leading-none">{label}</span>
                        </div>
                    ))}
                </div>
            ...
            {/* Legend */}
            <div className="flex items-center justify-end gap-1.5 mt-2.5 text-xs text-white/70">
                <span>Less</span>
                ...
                <div className="w-2.5 h-2.5 rounded-[2px]" style={{ backgroundColor: 'var(--primary)' }} />
                <div className="w-2.5 h-2.5 rounded-[2px]" style={{ backgroundColor: 'var(--primary-light)' }} />
                <span>More</span>
            </div>
        </div>
    )
