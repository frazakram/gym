...
const TIPS = [
  { icon: StretchHorizontal, color: 'text-[#22D3EE]', bg: 'bg-[#22D3EE]/10', text: '10 min stretching or foam rolling' },
  { icon: Droplets, color: 'text-[#60A5FA]', bg: 'bg-[#60A5FA]/10', text: 'Stay hydrated — aim for 2.5L+ today' },
  { icon: Moon, color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10', text: '7-9 hours of sleep for muscle recovery' },
]
    ...
    <GlassCard className="p-4">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-7 h-7 rounded-lg bg-[#8B5CF6]/15 flex items-center justify-center">
          <Heart className="w-3.5 h-3.5 text-[#A78BFA]" />
        </div>
        ...
        {TIPS.map((tip, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2.5">
              ...
              <p className="text-xs text-[#C4B5FD]">{tip.text}</p>
            </div>
          )
        })}
      </div>
